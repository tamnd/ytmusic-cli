...
describes what users get. Pick a format with `-o`, or let ytmusic choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ytmusic <command> -o table   # aligned columns for reading
ytmusic <command> -o jsonl   # one JSON object per line, for piping
ytmusic <command> -o json    # a single JSON array
ytmusic <command> -o csv     # spreadsheet friendly
ytmusic <command> -o tsv     # tab-separated
ytmusic <command> -o url     # just the URL column
ytmusic <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ytmusic <command> --fields id,title,url
```
...
```bash
ytmusic <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ytmusic <command>            # a table, because this is a terminal
ytmusic <command> | wc -l    # JSONL, because this is a pipe
```
